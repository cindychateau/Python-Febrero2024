...
class User:
    ...
    def save(cls, form):
        ...
        result = connectToMySQL('esquema_recetas').query_db(query, form) #EL id del registro que creamos
        return result
    ...
    def validate_user(form):
        ...
        results = connectToMySQL('esquema_recetas').query_db(query, form)
        if len(results) >= 1:
    ...
    def get_by_email(cls, form):
        ...
        query = "SELECT * FROM users WHERE email = %(email)s"
        results = connectToMySQL('esquema_recetas').query_db(query, form) #LISTA
        if len(results) == 1:
    ...
    def get_by_id(cls, form):
        ...
        result = connectToMySQL('esquema_recetas').query_db(query, form)#LISTA de diccionarios.. Que tiene solo 1 posición
        user = cls(result[0])
        return user

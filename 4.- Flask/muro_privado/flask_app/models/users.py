...
class User:
    ...
    def save(cls, form):
        ...
        result = connectToMySQL('esquema_muroprivado').query_db(query, form) #EL id del registro que creamos
        return result
    ...
    def get_by_email(cls, form):
        ...
        query = "SELECT * FROM users WHERE email = %(email)s"
        results = connectToMySQL('esquema_muroprivado').query_db(query, form) #LISTA
        if len(results) == 1:
    ...
    def get_by_id(cls, form):
        ...
        result = connectToMySQL('esquema_muroprivado').query_db(query, form)#LISTA de diccionarios.. Que tiene solo 1 posición
        user = cls(result[0])
        return user
    ...
    def get_all(cls):
        ...
        for u in results:
            ...
            users.append(cls(u)) #cls(u)- Crea la instancia en base al dicc. users.append agrega esa instancia a la lista de usuarios
        return users #Regresamos una lista de instancias user